...
- `delegateTool()` exposes another `AgentLike` as a tool.
...
Use `createTeam()` when you want to talk to one `AgentLike` while it coordinates with named members internally. The team automatically injects delegate tools for its members and drives the mailbox runtime when you call `team.query()` or `team.prompt()`.
...
`createTeam()` injects member delegate tools into the lead. Member agents that can accept tools also receive mailbox tools:

...
`agentTool()` accepts the same declaration as `AgentToolOptions.isConcurrencySafe` (*requires 0.24.0 or later*), so a supervisor can fan out independent delegations in one turn. The predicate receives the tool's parsed input — the `inputSchema`-validated value for typed delegation, the `AgentToolInput` shape otherwise. Note the target's lifecycle: an `AgentSpec` spawns a fresh session per call, while an `AgentLike` target keeps history across calls and is usually not safe to call concurrently.

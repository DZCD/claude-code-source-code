...
AgentLattice supports a lightweight Skill API. It does not require the Claude Code runtime, and it does not load Claude Code plugins.

## Create a skill in code

```ts
import { createAgent, skill } from "agent-lattice";
...
```ts
import { createAgent, loadSkill } from "agent-lattice";

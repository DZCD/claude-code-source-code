...
`createAgentWorkspaceTools()` returns the same `ToolDefinition[]` shown in the table below (`createAgent()` calls it internally). Call it directly when you want to assemble the tool list yourself instead of using the default workspace setup.

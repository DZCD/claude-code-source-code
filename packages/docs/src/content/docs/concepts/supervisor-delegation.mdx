...
`AgentToolOptions.isConcurrencySafe` opts delegated calls into parallel execution under the default `toolConcurrency.mode: "safe"`, so a supervisor can fan out independent delegations in one turn. The predicate receives the tool's parsed input. A spec target spawns a fresh session per call, but a session target keeps history across calls and is usually not safe to call concurrently — see [Spec or session target](#spec-or-session-target). *Requires 0.24.0 or later.*

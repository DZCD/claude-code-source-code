...
Skill 是一段可复用的说明文档。它比 Claude Code plugin 更轻：SDK 只读取说明，并在相关请求里注入给模型，不依赖 Claude Code 的 plugin runtime。

## 内存中定义 skill

```ts
import { skill } from "@npm-while1/claude-agent-sdk";
...
```ts
import { loadSkill } from "@npm-while1/claude-agent-sdk";

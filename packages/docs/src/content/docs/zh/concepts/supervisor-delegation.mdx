...
`AgentToolOptions.isConcurrencySafe` 让委派调用可以在默认的 `toolConcurrency.mode: "safe"` 下并发执行，supervisor 因此能在一轮中并行发起多个互不依赖的委派。判断函数收到的是工具解析后的输入。模板目标每次调用都会开启新会话，而会话目标会跨调用保留历史，通常不适合并发调用——见 [模板目标还是会话目标](#模板目标还是会话目标)。*需要 0.24.0 及以上版本。*
...
自 0.23.0 起还固定了一种组合：**target 未声明 schema、`agentTool()` 显式声明**时，装配期放行（没有可核对的对象），`ask` 会把子级的 `structuredResult` 按父级这份 schema 校验后以 JSON 返回。这适合子级用自造的 `endTurn` + `structuredResult` 工具做 schema 之外的领域校验（如引用真实性）、契约由父级单方面声明的场景。

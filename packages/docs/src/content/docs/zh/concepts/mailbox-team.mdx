...
handoff 工作默认串行执行。如果多个独立成员可以同时工作，通过 `runner.maxConcurrentWorkItems` 设置并发上限：
...
高级调用方也可以直接向 `createTeamRunner()` 传入同名参数。该值必须是正整数，默认值为 `1`。
...
默认情况下，每个已接收的 handoff 都是相互隔离的工作项。如果某个成员返回 `MaxTurnsError`、`APIError` 等 Agent 错误，runtime 会：

1. 把该成员的工作项标记为 `failed`；
2. 输出一条 `subtype: "failed"` 的 `team_message` 事件，并附带归一化错误；
3. 向 lead 发送失败报告，同时把错误写入 `message.metadata`；
4. 继续执行其它已接收的 handoff；
5. 把成功和失败报告一起交给 lead。

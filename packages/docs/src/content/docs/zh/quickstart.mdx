---
title: 快速开始
description: 安装 AgentLattice，并运行一个最小 Agent。
---
...
图片或文档输入要传 content block 数组，不要塞进普通字符串。SDK 会在 agent loop
中原样转发 Anthropic 兼容的 `image` 和 `document` block。
...
- 需要一次性子智能体协作，阅读 [Supervisor 子智能体委派](/zh/concepts/supervisor-delegation/)。
- 需要持久团队协作，阅读 [Mailbox Team 持久团队](/zh/concepts/mailbox-team/)。
